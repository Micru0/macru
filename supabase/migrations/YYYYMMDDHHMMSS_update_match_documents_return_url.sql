-- Drop the existing function signature first (using the signature from the previous migration)
-- Ensure the signature matches EXACTLY the one currently in your database
...
-- Create or replace the function with the new return signature including source_url
...
-- ADD source_url to the RETURNS TABLE definition
RETURNS TABLE (
  id uuid,               -- Chunk ID
...
BEGIN
  ...
  SELECT
    ...
    d.location,
    d.participants,
    d.source_url -- SELECT THE ADDED source_url COLUMN <<<< ADDED
  FROM
    embeddings e
...
-- Update the comment on the function with the new parameter list including source_url
COMMENT ON FUNCTION match_documents(
  vector(768), float, integer, uuid, text[], timestamptz, timestamptz,
  timestamptz, timestamptz, timestamptz, timestamptz, text, text, text, text[]
) IS 'Search for similar document chunks using vector similarity AND structured metadata filters, returning chunk data AND structured metadata AND source URL from the parent document. Uses SECURITY DEFINER.'; 